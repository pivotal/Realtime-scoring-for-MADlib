...
class myDataFiles:
    ...
    def __init__(self):
        ...
        if (os.path.isfile('./account_location.json')):
            logging.info("Found existing accounts_location.json, So not generating mappings......")
            with open('account_location.json') as data_file:
                self.accounts_location = json.load(data_file)
        else:
            # Setup progress bar
            ...
            for a in self.accounts:
                ...
                pbar.update()
            f = open("account_location.json",'w')
            f.write(json.dumps(self.accounts_location))
            f.close()
            manager.stop()
